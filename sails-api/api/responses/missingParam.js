 ...
 *     // -or-
 *     return res.missingParam(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
 ...
 */
 ...
 module.exports = function missingParam(optionalData) {
    ...
    var resp = {};
        resp.status = 'false';
        resp.response_msg = 'Fill required parameters';
        return res.json( resp );
  
    // If no data was provided, use res.sendStatus().
    if (optionalData === undefined) {
      sails.log.info('Ran custom response: res.missingParam()');
    ...
    // except in production use res.sendStatus().
      ...
      sails.log.info('Custom response `res.missingParam()` called with an Error:', optionalData);
